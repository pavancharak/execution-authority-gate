...
- **Stored? PARTIAL, WEAKLY.** Every decision (not just BLOCK, ALLOW/FLAG/BLOCK
  ...
  `pipeline/decisions/pipeline_decisions.json`, not a log that only appends. Running the
  ...
  - Critically, `pipeline/decisions/` is **excluded from git**
    ...
    git ignored JSON file that gets overwritten on every write.
- **Signed? YES, but not BLOCK specific.** `sign_pipeline_decision()`
  ...
  signing path specific to BLOCK; a BLOCK gets no stronger cryptographic treatment than
  an ALLOW.
- **Verifiable afterward? PARTIAL.** `signature_verifier.verify_record()`
  ...
  - There is **no dedicated verify endpoint or CLI script**. A grep across the repo
    ...
    as a later, separate reverification.
...
§2) and reverified the same `decision` object's signature. It still returned
...
"verifiable" means a function exists and works within one process, not that any operator,
judge, or downstream system has a durable record, one that only appends and can be
fetched independently, to check against later. There is no durability guarantee specific
to BLOCK beyond what every ALLOW decision also gets.
...
| `month_to_date_total` | Computed in memory by the pipeline itself as a running sum per customer over the test set walk order (`run_pipeline.py:96,106,115`) | **N**, not derived from any ledger or authoritative balance; explicitly documented as "not a claim about real elapsed time" (`run_pipeline.py:91-95`); not persisted at all |
...
| `fraud_score` | `model.predict_proba(X_test)[:, 1]` (`detector.py:76`), a `RandomForestClassifier` trained fresh each pipeline run (`detector.py:49-59`) | **Weakly Y for the number itself, N for recomputing it.** The score value IS inside the signed envelope (`fraud_score` field, `authority_signer.py:168`), so it's tamper evident as a *number*. But it cannot be **recomputed or rederived** later to check the model didn't hallucinate it, because: (a) the six raw features that produced it aren't stored per transaction, (b) `detect/models/*.pkl` is git ignored (`.gitignore:224`) and gets overwritten every run (`detector.py:62-65`), and (c) the source dataset comes from an LLM call at `temperature=0.9` (`generate/src/llm_client.py:114,127`), which is not deterministic even with the same prompt, and that dataset (`generate/data/*.json`) is also git ignored (`.gitignore:223`) |
...
dataset) are all ephemeral and git ignored. Every other signal that feeds a
...
- **Executed at:** **NOT IMPLEMENTED anywhere in this repo.** After signing, the
  ...
  grep across the whole repo for `execute|charge|transfer|payment_api|move_money|debit|stripe|
...
- **`ARCHITECTURE.md` overstates this layer.** `ARCHITECTURE.md:57` shows a step
  ...
  externally**." A grep across the whole repo for `authority_gateway` and `.enforce(` matched
...
- **Caller auth mechanism: NONE.** The only code facing the network is
  ...
  verification, no authorization of any kind at the request level. `CORS(app)` at
  ...
  run_pipeline.py`) with no concept of caller identity at all.
...
- **What looks like caller auth but isn't:** `issue_agent_token()`
  ...
  (`generate/src/fraud_agents.py:73,91-92`). It is not an authorization
...
stays within its own budget that it requested itself, not to gate real callers.
...
- **BLOCK decision tests:** ~11 test functions directly assert or exercise a BLOCK
  ...
  fixture entries).
- **Verification (reverify a signed BLOCK) tests:** 5 test functions call
  ...
  includes some BLOCKs, but doesn't isolate or target BLOCK specifically.

**Gaps:**
- **No test reverifies a BLOCK decision after it has been written to and read
  back from `pipeline_decisions.json`.** Every verification test in the suite signs and
  verifies in the same step in memory, inside `isolated_sign_env` (an ephemeral
  fixture using a tmp dir, per `test_signer.py`'s docstring at lines 1-6). None load the
  real, on disk `pipeline/decisions/pipeline_decisions.json` and check a record's
  signature against the persisted public key. That is the exact "verify it later" scenario
...
Execution-Authority-Gate is a **decision only authorization layer for fraud
and mandate checks**, not an execution layer, and its own `ARCHITECTURE.md` overstates what
...
genuine decision made across two layers (`detect` + `mandate`) combined so either layer can
...
short, EAG is a credible prototype for decisioning that combines fraud detection
and authorization, with real cryptography where it applies it, but it is not yet a system whose BLOCK
outcomes are durable, whose inputs can be independently reverified, or that has any
