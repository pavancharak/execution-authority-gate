...
This document describes what would need to be true for Parmana Authority
...
Parmana is a **decision-ready authorization layer**: it detects, checks
...
| Layer | Work | Budget |
|---|---|---|
| Detect | `RandomForestClassifier.predict_proba` on 6 features, `detect/src/detector.py` | <100ms |
| Mandate | 4 deterministic rule checks against a cached customer mandate, `mandate/src/mandate_checker.py` | <50ms |
| Sign | One Ed25519 sign operation, `sign/src/authority_signer.py` | <10ms |
| Execute | Signature verify + idempotency lookup + webhook dispatch, `sign/src/decision_executor.py` | <20ms (excluding the downstream processor's own latency, which is out of this system's control) |
| **Total** | | **<200ms**, not counting network round trip to the payment processor itself |

These are engineering estimates based on the operations involved (a
single forest inference, four in-memory comparisons, one signature),
...
a single row is sub-millisecond on commodity hardware in isolation; the
100ms budget above is deliberately generous to leave headroom for
feature assembly and network overhead in a real request path.
...
- **Detect**: stateless once the model is loaded (`detect/src/detector.py::load_model`).
  ...
  balancer, not per-transaction retraining, is the scaling lever.
...
- **Mandate**: each customer's mandate (`mandate/src/mandate_checker.py::derive_mandate_from_history`)
  is derived once from historical transactions and does not change
  per-transaction. In production this is a cache lookup (e.g. Redis,
  keyed on `customer_id`), not a recomputation from full history on
  every request. `month_to_date` and `tx_count_today` (currently
  in-memory dicts scoped to one `run_pipeline.py` process, see
  `pipeline/src/run_pipeline.py` lines 96-97) would need to move to a
  real per-customer counter store (e.g. Redis `INCR` with a
  day/month-scoped key) to survive across requests and instances.
- **Sign**: Ed25519 signing is single-purpose and fast, but the private
  ...
  in front of a single HSM/KMS-backed signing endpoint, not by
...
- **Executor**: `sign/src/decision_executor.py`'s idempotency check
  ...
  "Audit trail" below.
- **Target**: at 100M tx/day (~1,160 tx/sec average, with realistic
  peak multiples of 5-10x), the detect and mandate layers scale
  horizontally without architectural change; the signing and audit
  writes are the layers that need a single, durable, high-availability
...
```
   ...
   transaction  ───▶ │ Detection service │  stateless, autoscales
                     ...
                     │ (sign/src)        │  logical writer, HA via
                     │                   │  stateless replicas in front
                     └─────────┬─────────┘  of the HSM
                               │ signed decision
                 ┌─────────────┴─────────────┐
                 ▼                           ▼
...
- **Public key**: committed to git at `sign/tokens/authority_public_key.pem`
  ...
  explicitly excepts these two files from the otherwise-ignored
  `sign/tokens/*.pem` pattern). This is what makes verification
...
- **Caller auth secret**: `sign/src/caller_auth.py` uses a separate
  ...
  from the Ed25519 decision-signing key by design: a caller proving its
  own identity should never be the same secret that makes a decision's
  *content* authoritative (see `caller_auth.py`'s module docstring for
  why conflating the two would reintroduce self-authorization).
...
- **PCI-DSS**: the append-only audit trail (`pipeline/src/audit_trail.py`)
  ...
  invalidating the signature. This repo's mandate/detect layers do not
  themselves store cardholder data (PAN, CVV); a real deployment must
  separately confirm PCI-DSS scope for wherever `amount`, `merchant`,
  and similar transaction fields are persisted (currently the unsigned
  `ground_truth` sibling field, see `ARCHITECTURE.md`'s Verification
  section for what is and isn't inside the signed envelope).
...
- **GDPR**: the audit trail is the durable record a customer dispute or
  ...
  is currently unsigned and un-redacted, a production deployment needs
  ...
  harder (erasure of one record without breaking the trail's
  append-only property for every other record is an open design
...
1. Obtain a caller token via `POST /api/callers/token` with a registered
   ...
   not BLOCK).
2. Confirm the token's `expires_at` and re-request before expiry; tokens
   are not silently renewed.
3. Fetch the authority's public key from `sign/tokens/authority_public_key.pem`
   ...
   verify it out-of-band matches what your integration expects, not just
   what the API returns at runtime.
...
6. Treat `ALREADY_EXECUTED` as a success, not an error: it means this
   ...
   idempotency guarantees you will not double-settle it.
...
8. Never call your own settle/deny logic directly from a signed
...
10. Do not treat a `FLAG` → `step_up_auth` action as equivalent to
...
12. Verify every decision's signature yourself before acting on it, even
    though `enforce_decision` already does this server-side; a
    defense-in-depth integration does not trust a single verification
    point.
...
15. Before going live, run this repo's test suite
    (`pytest tests/ -v`) against your fork/deployment to confirm no
...
(a single JSON array, fully overwritten on every pipeline run, and
git-ignored, see `EAG-AUDIT-GAPS.md` section 1), and the JSONL file is not
...
as the primary store, with the JSONL file retained as a portable,
dependency-free backup and as the artifact judges/auditors without
